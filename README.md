...
| `encode_fasta` | no | Whether to run `encodeFasta` on the variant peptide FASTA called by `callVariant`, after `filterFasta` and `splitFasta` if applicable. Defaults to `false`. |
| `decoy_fasta` | no | Whether to run `decoyFasta` on the variant peptide FASTA called by `callVariant`, after `filterFasta`, `splitFasta` and `encodeFasta` if applicable. Defaults to `false`. |
