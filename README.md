...
| `split_database` | no | Whether to run `splitDatabase` on the variant peptide FASTA called by `callVariant`. Defaults to `false`. |
| `filter_fasta` | no | Whether to run `filterFasta` on the variant peptide FASTA called by `callVariant`. Defaults to `false`. |
...
The variables below are set under tool specific namespaces. See [this](test/test-integration/test.config) example config to see how they are set. If the too is not used, the namespace does not needs to be set. For example, if REDItools results is not included in the input CSV, `moPepGen arseREDItools` won't be called, so the `parseREDItools` namespace does not need to be present in the config file.
