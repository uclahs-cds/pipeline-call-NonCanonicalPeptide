...
| `merge_variant_noncoding` | no | Merge the variant and noncoding FASTA file. Choices: ['yes', 'no', 'both']. Defaults to 'no' |
| `encode_fasta` | no | Whether to run `encodeFasta` on the variant peptide FASTA called by `callVariant` （run once after `filterFasta` and `splitFasta`, if used). Defaults to `false`. |
| `decoy_fasta` | no | Whether to run `decoyFasta` on the variant peptide FASTA called by `callVariant` (run once after `filterFasta`, `splitFasta` and `encodeFasta`, if used). Defaults to `false`. |
